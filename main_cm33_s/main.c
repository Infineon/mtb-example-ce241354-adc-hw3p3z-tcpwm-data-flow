...
int main(void)
{
    ...
    int32_t              *hw_filter_out      = adc_filter_out + 1;
    ...
    printf("\x1b[2J\x1b[;H");
    printf("************************************************************\r\n");
    printf("PSOC Control C3M/P8: ADC-HW3P3Z-TCPWM dedicated datapath\r\n");
    printf("************************************************************\r\n\n");
    ...
    Cy_PPCA_EPU_Combo_Configure(PPCA_EPU_EPU, ADC_TRIG_INDEX, &ADC_TRIG_combo_config);
    ...
    Cy_PPCA_ADC_Filter_Init(ADC_FILTER_HW, &ADC_FILTER_config);
    ...
    for (;;)
    {
        ...
        {
            printf("\r\n ADC out: %X, ADC filter out: %X, HW Filter out: %X\r\n", (unsigned int)*adc_data, (unsigned int)*adc_filter_out, (unsigned int)*hw_filter_out);
